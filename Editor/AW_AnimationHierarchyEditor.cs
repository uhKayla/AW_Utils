...
// Moved namespaces and renamed class to avoid conflicts.
{
    /// <summary>
    /// Original script fork by CuteWaterBeary on Github. License is Unlicense, located in the repository linked below.
    /// Thanks!
    /// https://github.com/CuteWaterBeary/Unity-Animation-Hierarchy-Editor/
    /// </summary>
    public class AW_AnimationHierarchyEditor : EditorWindow
    {

        #region Automated Variables

        private readonly Dictionary<string, List<EditorCurveBinding>> paths =
            new Dictionary<string, List<EditorCurveBinding>>();
        ...
        [MenuItem("ANGELWARE/Tools/Animation Hierarchy Editor")]
        // Menu location moved to containerise scripts.
        static void ShowWindow() => GetWindow<AW_AnimationHierarchyEditor>();
        ...
        void OnGUI()
        {
            ...
            {
                animatorObject =
                    (Animator)EditorGUILayout.ObjectField("Root Animator:", animatorObject, typeof(Animator), true);

                using (new GUILayout.HorizontalScope())
                {
                    if (animationClips.Length == 1)
                        animationClips[0] = (AnimationClip)EditorGUILayout.ObjectField("Target Clip:",
                            animationClips[0], typeof(AnimationClip), false);
                    else
                    {
            ...
            using (new GUILayout.VerticalScope("helpbox"))
            {
                using (new EditorGUILayout.HorizontalScope("box"))
                {
                    ...
                    using (new EditorGUI.DisabledScope(string.IsNullOrEmpty(oldPathValue)))
                        if (GUILayout.Button(new GUIContent("Replace Path",
                                "Replaces the old string (left) with the new string (right). Field on the left will be used as a Regex Pattern if Use Regex (@) is enabled.")))
                            UpdatePath(oldPathValue, newPathValue, true);
                    regexReplace = GUILayout.Toggle(regexReplace, new GUIContent("@", "Use Regex"), "button",
                        GUILayout.Width(22));
                }
            ...
            try
            {
                ...
                for (int clipIndex = 0; clipIndex < animationClips.Length; clipIndex++)
                {
                    ...
                    foreach (var c in curves)
                    {
                        ...
                        {
                            if (identicalMayExist && curves.Any(c2 =>
                                    c2.path == newPath && c2.type == c.type && c2.propertyName == c.propertyName))
                                Debug.LogWarning(
                                    $"Identical settings curve already exists. Skipping curve.\nPath: {c.path}\nType: {c.type.Name}\nProperty: {c.propertyName}");
        ...
        private GameObject FindObjectInRoot(string path) =>
            animatorObject ? animatorObject.transform.Find(path)?.gameObject : null;
